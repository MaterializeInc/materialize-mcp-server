...
class ToolMetrics:
    ...
    recent_calls: Deque[Dict[str, Any]] = field(
        ...
        async with self._lock:
            ...
            if success:
                ...
                self.connection_metrics.avg_query_duration_ms = (
                    current_avg * (total_queries - 1) + duration_ms
                ) / total_queries
            else:
...
class HealthChecker:
    ...
    async def perform_health_checks(self) -> Dict[str, bool]:
        ...
        failed_checks = [
            name for name, status in self.health_status.items() if not status
        ]
        if failed_checks:
...
async def track_tool_call(
    metrics_collector: MetricsCollector, tool_name: str, logger: logging.Logger
):
    """
