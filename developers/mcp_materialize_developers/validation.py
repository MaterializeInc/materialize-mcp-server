...
class InputValidator:
    ...
    CLUSTER_SIZE_PATTERN = re.compile(
        r"^(\d+cc|xsmall|small|medium|large|xlarge|2xlarge|3xlarge|4xlarge|5xlarge|6xlarge)$"
    )
    ...
    def validate_identifier(value: Any, field_name: str) -> ValidationResult:
        ...
        if value.lower() in sql_keywords:
            result.add_error(
                field_name, f"'{value}' is a reserved SQL keyword and must be quoted"
            )
    ...
    def validate_cluster_size(
        value: Any, field_name: str = "cluster_size"
    ) -> ValidationResult:
        ...
        if not InputValidator.CLUSTER_SIZE_PATTERN.match(value):
            result.add_error(
                field_name,
                "must be either a number followed by 'cc' (e.g., '100cc') or a predefined size (xsmall, small, medium, large, xlarge, 2xlarge, 3xlarge, 4xlarge, 5xlarge, 6xlarge)",
            )

        return result

    @staticmethod
    def validate_sql_statements(
        value: Any, field_name: str = "sql_statements"
    ) -> ValidationResult:
        ...
        for i, stmt in enumerate(value):
            if not isinstance(stmt, str):
                result.add_error(
                    f"{field_name}[{i}]", f"must be a string, got {type(stmt).__name__}"
                )
                continue
    ...
    def validate_schema_name(
        value: Any, field_name: str = "schema"
    ) -> ValidationResult:
        ...
        if not InputValidator.SCHEMA_PATTERN.match(value):
            result.add_error(
                field_name,
                "must contain only letters, digits, underscores, and dots, and start with a letter or underscore",
            )
    ...
    def validate_port(value: Any, field_name: str = "port") -> ValidationResult:
        ...
        if value < InputValidator.MIN_PORT or value > InputValidator.MAX_PORT:
            result.add_error(
                field_name,
                f"must be between {InputValidator.MIN_PORT} and {InputValidator.MAX_PORT}",
            )
    ...
    def validate_pool_size(
        value: Any, field_name: str = "pool_size"
    ) -> ValidationResult:
        ...
        if value < InputValidator.MIN_POOL_SIZE or value > InputValidator.MAX_POOL_SIZE:
            result.add_error(
                field_name,
                f"must be between {InputValidator.MIN_POOL_SIZE} and {InputValidator.MAX_POOL_SIZE}",
            )

        return result

    @staticmethod
    def validate_threshold_seconds(
        value: Any, field_name: str = "threshold_seconds"
    ) -> ValidationResult:
    ...
    def validate_create_cluster_args(args: Dict[str, Any]) -> ValidationResult:
        ...
        cluster_result = InputValidator.validate_identifier(
    ...
    def validate_run_sql_transaction_args(args: Dict[str, Any]) -> ValidationResult:
        ...
        sql_result = InputValidator.validate_sql_statements(
        ...
        if isolation_level is not None:
            ...
            if (
                not isinstance(isolation_level, str)
                or isolation_level.lower() not in valid_levels
            ):
                result.add_error(
                    "isolation_level", f"must be one of: {', '.join(valid_levels)}"
                )
...
def validate_and_sanitize_args(tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if tool_name == "create_cluster":
        ...
        return {
            ...
            "size": args[
                "size"
            ],  # Size is validated but not sanitized as it's not used in dynamic SQL
        }
        ...
        return {
            ...
            "sql_statements": args[
                "sql_statements"
            ],  # SQL statements are validated but not modified
            "isolation_level": args.get("isolation_level"),
        }
