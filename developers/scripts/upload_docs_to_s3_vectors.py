...
async def main():
    ...
    if not (
        os.getenv("AWS_ACCESS_KEY_ID", "AKIAV2KIV5LP5E5UBPHG")
    ...
    try:
        ...
        store = S3VectorBucketStore(bucket_name=bucket_name, region=region)
        ...
        documents = []
        async for doc_batch in temp_searcher._scrape_documentation_generator(
            max_pages=500
        ):
        ...
        print()
        print(
            "2. The documentation will be automatically loaded from S3 Vector Buckets"
        )
