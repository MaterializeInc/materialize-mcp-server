...
class S3VectorBucketStore:
    ...
    async def add_document(self, doc_data: Dict[str, Any]):
        ...
        await asyncio.get_event_loop().run_in_executor(
            self.executor, put_vectors_sync
        )
        
        logger.debug(f"Added document to vector index: {doc_data['title']}")
    
    async def bulk_add_documents(self, documents: List[Dict[str, Any]], batch_size: int = 100):
    ...
    async def search(self, query: str, limit: int = 5, section_filter: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if section_filter:
            metadata_filter = {
                'section': {'StringEquals': section_filter}
            }
    ...
    async def get_bucket_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            bucket_response, index_response = await asyncio.get_event_loop().run_in_executor(
...
# Global instance
_s3_vector_bucket_store = None

async def get_s3_vector_bucket_store(
    bucket_name: str = "materialize-docs-vectors",
    region: str = "us-east-1"
) -> S3VectorBucketStore:
    """Get or create the global S3 Vector Bucket store instance."""
    global _s3_vector_bucket_store
    if _s3_vector_bucket_store is None:
        _s3_vector_bucket_store = S3VectorBucketStore(
            bucket_name=bucket_name,
            region=region
        )
        await _s3_vector_bucket_store.initialize()
    return _s3_vector_bucket_store